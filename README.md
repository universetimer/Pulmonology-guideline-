...
```
respiratory-guideline/      # Cloudflare Pages가 그대로 deploy
├── index.html              # 랜딩 페이지
...
├── files/                  # 25MB 초과 파일 (git 추적 안 함, GitHub Release 사용)
│   └── manual-v18-with-2026-supplement.docx   (≈ 42 MB · 별도 호스팅)
...
3. Build 설정:
   ...
   - **Build command**: (비워둠)
   - **Build output directory**: (비워둠 — 저장소 루트가 곧 사이트 root)
   - **Root directory**: (비워둠)
...
브라우저에서 `index.html`을 열거나, 간단한 정적 서버 실행:
